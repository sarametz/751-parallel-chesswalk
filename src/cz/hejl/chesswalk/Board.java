...
public class Board {
    ...
    private boolean squareAttacked(int square, int by) {
        ...
        for (int i = 0; i < queenDeltas.length; i++) {
            ...
            while (true) {
                ...
                if (board0x88[square2] * by > 0
                        && Math.abs(board0x88[square2]) >= 3
                        && Math.abs(board0x88[square2]) <= 5) {
    ...
    public String toFEN() {
        ...
        if (enPassant == -1)
            ...
            FEN.append(fileSymbols[enPassant & 7]
                    + Integer.toString(enPassant / 16 + 1) + " ");
    ...
    public void undoMove(Move move) {
        ...
        if ((move.piece == 1 || move.piece == -1)
                && (move.from & 7) != (move.to & 7) && move.capture == 0) {
            if (move.piece == 1)
