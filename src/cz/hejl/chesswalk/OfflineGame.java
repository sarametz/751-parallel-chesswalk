...
public class OfflineGame extends Activity implements MoveListener, OnClickListener {
	...
	public void onClick(View v) {
		if (v.getId() == R.id.btNewGame) {
			...
			if (moveHistory.size() > 0) {
				...
				if (chessBoard.getWhiteEnabled() && whiteHuman == false || chessBoard.getBlackEnabled()
						&& blackHuman == false) {
					if (moveHistory.size() > 0) {
						chessBoard.undoMove(moveHistory.get(moveHistory.size() - 1));
						moveHistory.remove(moveHistory.size() - 1);
	...
	protected void onPause() {
		...
		for (int i = 0; i < moveHistory.size(); i++) {
			...
			db.execSQL("INSERT INTO " + MOVES_TABLE_NAME + " VALUES (" + i + ", " + move.from + ", "
					+ move.to + ", " + move.piece + ", " + move.capture + ", " + move.enPassant + ", "
					+ (move.castlingRights[0] ? 1 : 0) + ", " + (move.castlingRights[1] ? 1 : 0) + ", "
					+ (move.castlingRights[2] ? 1 : 0) + ", " + (move.castlingRights[3] ? 1 : 0) + ", '"
					+ move.info + "');");
		}
	...
	protected void onResume() {
		...
		if (databaseList().length > 0) {
			...
			SQLiteDatabase db = openOrCreateDatabase(DATABASE_NAME, 0, null);
			Cursor cursor = db.query(MOVES_TABLE_NAME, new String[] { "id", "sq_from", "sq_to", "piece",
					"capture", "en_passant", "castling0", "castling1", "castling2", "castling3", "info" },
			...
			if (cursor.moveToFirst()) {
				...
				do {
					Move move = new Move(cursor.getInt(columnFrom), cursor.getInt(columnTo), cursor
							.getInt(columnPiece), cursor.getInt(columnCapture), cursor
							.getInt(columnEnPassant), new boolean[] { cursor.getInt(columnCastling0) != 0,
							cursor.getInt(columnCastling1) != 0, cursor.getInt(columnCastling2) != 0,
							cursor.getInt(columnCastling3) != 0 }, cursor.getString(columnInfo));
			...
			if (whiteHuman == false && chessBoard.getWhiteEnabled() && gameEnded == false)
				computerMove();
	...
	public void pieceMoved(Move move) {
		...
		if (chessBoard.board.toMove == 1 && whiteHuman == false || chessBoard.board.toMove == -1
				&& blackHuman == false) {
			computerMove();
		}
	...
	private class EngineTask extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... nothing) {
			int intMove = getBestMove(chessBoard.board.toFEN(), 10, strength * 1000);
			return intMove;
			/*
		...
		protected void onPostExecute(Integer intMove) {
			Move move = new Move(chessBoard.board, intMove & 127, (intMove >> 7) & 127);
