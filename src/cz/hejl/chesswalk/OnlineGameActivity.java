...
public class OnlineGameActivity extends Activity implements MoveListener, OnlineGameListener, OnClickListener {
	...
	public void onChat(String message) {
		...
		SpannableString ss = new SpannableString(opponentName.substring(0, spaceIndex) + ": " + message);
		showMessage(ss.toString());
		ss.setSpan(new StyleSpan(Typeface.BOLD), 0, spaceIndex + 2, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		OnlineGameState onlineGameState = (OnlineGameState) getIntent().getSerializableExtra(
				"onlineGameState");
		...
		if (savedInstanceState != null) {
			...
			chessBoard.setBlackEnabled(savedInstanceState.getBoolean("blackAllowed"));
			chessBoard.setWhiteEnabled(savedInstanceState.getBoolean("whiteAllowed"));
			chessBoard.setFlipped(savedInstanceState.getBoolean("flipped"));
			chessBoard.setLastMoveFrom(savedInstanceState.getInt("lastMoveFrom"));
			chessBoard.setLastMoveTo(savedInstanceState.getInt("lastMoveTo"));
			sideToMove = (chessBoard.getBlackEnabled() || chessBoard.getWhiteEnabled()) ? 1 : -1;
			...
			tvYourMove.setText(savedInstanceState.getCharSequence("tvYourMoveText"));
	...
	public void onDrawAnswer(int answer) {
		if (answer == FicsParser.ACCEPT)
			tvInfo.setText(MessageFormat.format(getString(R.string.acceptsDraw), opponentName));
		else if (answer == FicsParser.DECLINE)
			tvInfo.setText(MessageFormat.format(getString(R.string.delinesDraw), opponentName));
	}

	// -------------------------------------------------------------------------------------------------------

	public void onDrawOffer() {
		new AlertDialog.Builder(this).setMessage(opponentName + " offers you a draw.").setPositiveButton(
	...
	public Dialog onCreateDialog(int dialogId) {
		if (dialogId == DIALOG_CHAT) {
			...
			builder.setMessage(getString(R.string.doYouWantToResign)).setCancelable(true).setPositiveButton(
					getString(R.string.btResign), new DialogInterface.OnClickListener() {
	...
	public void onOnlineMove(OnlineGameState onlineGameState) {
		...
		// update clocks
		myTime = flipped ? onlineGameState.blackTime : onlineGameState.whiteTime;
		opponentTime = flipped ? onlineGameState.whiteTime : onlineGameState.blackTime;
		tvMyClocks.setText(formatSeconds(myTime));
		tvOpponentClocks.setText(formatSeconds(opponentTime));
	...
	public void onRatingChange(int[] ratings) {
		showMessage(matchEnd.message + ". Your rating: " + ratings[0] + " > " + ratings[1]);
	}
	...
	public void pieceMoved(Move move) {
		...
		chessBoard.setWhiteEnabled(false);
		tvYourMove.setText("");
		chessClient.write(files[move.from & 7] + (move.from / 16 + 1) + "-" + files[move.to & 7]
				+ (move.to / 16 + 1) + "\n");
	}
	...
	private void resign() {
		new AlertDialog.Builder(this).setMessage(R.string.doYouWantToResign).setPositiveButton(
				R.string.btResign, new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						chessClient.write("resign\n");
					}
	...
	private class ClocksTask implements Runnable {
		...
		public void run() {
			...
			if (clockTime > 0)
				handler.postAtTime(this, startTime + (passedSeconds + 1) * 1000);
		}
	}
	...
	private class ChatDialog extends Dialog {
		...
		private void send() {
			...
			ss.setSpan(new StyleSpan(Typeface.BOLD), 0, me.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			chatAdapter.addMessage(ss);
		}
	...
	public static class MatchEnd {
		...
		public String getMessage(boolean flipped, Context context) {
			...
			if (type == RESIGNATION) {
				if (iWon)
					return MessageFormat.format(context.getString(R.string.matchEndHeResigned), opponent);
				else
				...
				if (iWon)
					...
					return MessageFormat.format(context.getString(R.string.matchEndHeCheckmated), opponent);
			} else if (type == DISCONNECTION) {
				if (iWon)
					return MessageFormat.format(context.getString(R.string.matchEndHeDisconnected), opponent);
				else
					return context.getString(R.string.matchEndIDisconnected);
			} else if (type == TIME) {
				if (iWon)
					return MessageFormat.format(context.getString(R.string.matchEndHeOutOfTime), opponent);
				else
					return MessageFormat.format(context.getString(R.string.matchEndIOutOfTime), opponent);
		...
		public MatchEnd(String white, String black, String message, String result) {
			this.white = white;
			this.black = black;
