...
public class NewGame extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tvDifficulty.setText(MessageFormat.format(getString(R.string.computerStrength), settings.getInt(
				"strength", 4)));
		...
		btStart.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!whiteHuman && !blackHuman) {
					Toast.makeText(NewGame.this, R.string.compVsCompToast, Toast.LENGTH_SHORT).show();
					return;
				}

				SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(NewGame.this);
		...
		skDifficulty.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
				tvDifficulty
						.setText(MessageFormat.format(getString(R.string.computerStrength), progress + 1));
			}
		});
	}
