...
public class FicsParser {
	...
	public FicsParser() {
		...
		creatingMatch = Pattern.compile("^Creating:.*?([0-9+]+).*?([0-9+]+)").matcher("");
		drawAnswer = Pattern.compile("(accepts|declines) the draw request").matcher("");
		drawOffer = Pattern.compile("\\w+ offers you a draw").matcher("");
		ratingLine = Pattern.compile("(Blitz|Standard|Lightning)\\s+([0-9+]+)").matcher("");
		matchEnd = Pattern.compile("^\\{Game.*?(\\w+) vs\\. (\\w+)\\) (.*)\\} (1-0|0-1|1/2-1/2|\\*)")
				.matcher("");
		ratingChange = Pattern.compile("rating adjustment.*?(\\d+).*?(\\d+)").matcher("");
		seekUnavailable = Pattern.compile("That seek is not available").matcher("");
		// 1 - id
		// 2 - rating
		// 3 - username
		// 4 - time
	...
	public OnlineGameActivity.MatchEnd parseMatchEnd(String line) {
		matchEnd.reset(line);
		if (matchEnd.find()) {
			return new OnlineGameActivity.MatchEnd(matchEnd.group(1), matchEnd.group(2), matchEnd.group(3), matchEnd
					.group(4));
		} else
			return null;
	}
	...
	public GameOffer parseSoughtLine(String line) {
		soughtLine.reset(line);
		if (soughtLine.find()) {
			int rating = -1;
			if (!soughtLine.group(2).equals("++++"))
				rating = Integer.parseInt(soughtLine.group(2));

			boolean rated;
			if (soughtLine.group(6).charAt(0) == 'r')
				rated = true;
			else
				rated = false;

			int color = 0;
			if (soughtLine.group(9) != null) {
				if (soughtLine.group(9).charAt(1) == 'w')
					color = 1;
				else
					color = -1;
			}

			GameOffer game = new GameOffer(soughtLine.group(1), rating, soughtLine.group(3), soughtLine.group(4),
					soughtLine.group(5), rated, color);
	...
	public OnlineGameState parseStyle12(String line) {
		style12.reset(line);
		if (style12.find()) {
			OnlineGameState onlineGameState = new OnlineGameState(style12);
